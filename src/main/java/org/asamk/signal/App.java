...
public class App {
    ...
    static ArgumentParser buildArgumentParser() {
        ...
        parser.addArgument("-e", "--env")
              .help("Set the env as either PROD, QA, or DEV.");
        parser.addArgument("-r", "--role")
              .help("Set the role as either ADMIN, VENDOR, CUSTOMER.");
        ...
        parser.addArgument("--disable-send-log")
                .help("Disable message send log (for resending messaging that recipient couldn't decrypt)")
                .action(Arguments.storeTrue());
