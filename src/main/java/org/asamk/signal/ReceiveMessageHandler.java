...
import com.undercurrent.system.messaging.inbound.InboundMessageReceiver;
  ...
  private void handleMessageInternal(MessageEnvelope envelope, Throwable exception) {
    ...
    if (envelope.data().isPresent()) {
      ...
      if (message.body().isPresent() ||
          ...
          message.payment().isPresent()) {
        InboundMessageReceiver.saveFromSignalCliDbus(envelope, m.getSelfNumber());
      }
//      if (message.payment().isPresent()) {
//        Messages.savePaymentFromSignalCliDbus(envelope, objectPath);
//      }
  ...
  private void printCallMessage(
    ...
    if (callMessage.hangup().isPresent()) {
      ...
      writer.println("Ice update messaging:");
  ...
  private void printSyncMessage(
    ...
    if (syncMessage.groups().isPresent()) {
      ...
      writer.println("Received sync read messaging list");
      ...
      writer.println("Received sync viewed messaging list");
    ...
    if (syncMessage.messageRequestResponse().isPresent()) {
      ...
      if (requestResponseMessage.person().isPresent()) {
        writer.indentedWriter()
              .println("For Person: {}", formatContact(requestResponseMessage.person().get()));
      }
    }
  }
  ...
  private void printReaction(
    ...
    writer.println("Target timestamp: {}",
                   DateUtils.formatTimestamp(reaction.targetSentTimestamp()));
  ...
  private void printSharedContact(final PlainTextWriter writer,
      final MessageEnvelope.Data.SharedContact contact) {
    writer.println("Name:");
  ...
  private void printMention(
    ...
    writer.println("- {}: {} (length: {})", formatContact(mention.recipient()), mention.start(),
                   mention.length());
  }
  ...
  private void printAttachment(PlainTextWriter writer, MessageEnvelope.Data.Attachment attachment) {
    ...
    if (attachment.uploadTimestamp().isPresent()) {
      writer.println("Upload timestamp: {}",
                     DateUtils.formatTimestamp(attachment.uploadTimestamp().get()));
    }
    ...
    if (attachment.size().isPresent() || attachment.preview().isPresent()) {
      writer.println("Size: {}{}",
                     attachment.size().isPresent() ? attachment.size().get() + " bytes"
                         : "<unavailable>",
    ...
    if (attachment.width().isPresent() || attachment.height().isPresent()) {
      writer.println("Dimensions: {}x{}", attachment.width().orElse(0),
                     attachment.height().orElse(0));
    }
